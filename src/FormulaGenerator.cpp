...
void FormulaGenerator::Generate(const std::vector<uint64_t>& inputs_, const std::vector<CEd>& fixedComps_, const std::vector<CEd>& bannedComps_)
{
	inputs = inputs_;
	fixedComps = fixedComps_;
	bannedComps = bannedComps_;

	// Add core network structural requirements and definitions
	InitializeVariables();
	AddValid();
	AddUsedDefinitions();
	...
	// Add some initial set of inputs, more can be added later
	for (uint64_t input : inputs)
		AddInput(input);
}

const Expression& FormulaGenerator::GetExpression() const
{
	return expr;
}
...
void FormulaGenerator::InitializeVVariables(VariableFamily& v, uint64_t input)
{
	...
	for (uint8_t i = 0; i < numZeros - 1; i++)
		v(d - 1, i) = falseVar;
	v(d - 1, numZeros - 1) = expr.NextVar();
	v(d - 1, numZeros) = expr.NextVar();
	for (uint8_t i = numZeros + 1; i < n; i++)
		v(d - 1, i) = trueVar;

	// Ensure that the output layer is sorted
	for (uint8_t i = 0; i < n; i++)
		v(d, i) = (i < numZeros) ? falseVar : trueVar;
}
...
void FormulaGenerator::AddInput(uint64_t input)
{
	...
	VariableFamily v{ d + 1U, n };
	InitializeVVariables(v, input);
	...
	for (uint8_t i = leadingZeros; i < (n - tailingOnes); i++)
	{
		...
		if (val)
		{
			...
			v(1, i) = -GetClauseVar(affectingComps);
		}
		else
		{
	...
	for (uint8_t k = 2; k < d; k++)
	{
		for (uint8_t i = leadingZeros; i < (n - tailingOnes); i++)
		{
			...
			expr.AddClause({ oneDown(k - 1, i, n - tailingOnes - 1), -v(k - 1, i), v(k, i) });
			...
			for (uint8_t j = i + 1; j < n - tailingOnes; j++)
			{
				...
				expr.AddClause({ -comps(k - 1, i, j), v(k - 1, j), -v(k, i) });

				// If both inputs equal "1"
				expr.AddClause({ -comps(k - 1, i, j), -v(k - 1, j), -v(k - 1, i), v(k, i) });
			}
			// What if the input is "0"? 
			// No comparator to lower indices -> output will be zero
			expr.AddClause({ oneUp(k - 1, leadingZeros, i), v(k - 1, i), -v(k, i) });

			for (uint8_t j = leadingZeros; j < i; j++)
			{
				// Comparator chosen, and other value="1" -> swap
				expr.AddClause({ -comps(k - 1, j, i), -v(k - 1, j), v(k, i) });

				// Comparator chosen, and other value="0" -> still "0"
				expr.AddClause({ -comps(k - 1, j, i), v(k - 1, j), v(k - 1, i), -v(k, i) });
			}
		}
	}
	...
	Var v0 = v(d - 1, numZeros - 1);
	Var v1 = v(d - 1, numZeros);
...
void FormulaGenerator::AddInputs(const std::vector<uint64_t>& inputs)
{
	for (uint64_t input : inputs) AddInput(input);
}
